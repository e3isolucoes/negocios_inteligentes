...
const SNIPPET = process.env.E3I_ADMIN_ORGANIZATIONS_SNIPPET || '/tmp/admin-organizations-server-snippet.txt';
const MARKER = 'E3I_ADMIN_ORGANIZATIONS_PATCH_V1';
...
if (!server.includes(MARKER)) {
  ...
  if (!snippet.includes(MARKER)) throw new Error('admin organizations snippet marker missing');
...
for (const required of [
  ...
  'ROOT_ADMIN_REQUIRED',
]) if (!patched.includes(required)) throw new Error(`admin organizations validation failed: ${required}`);
if (!patched.includes(LOGIN_MARKER)) throw new Error('login route was damaged');
console.log('PORTAL_ADMIN_ORGANIZATIONS_PATCH_OK');
