import { getAccessToken } from '../painel-obrigacoes/js/api/auth.js';
...
async function platformRequest(path, options = {}) {
  ...
  if (!token) throw new Error('Sessão não autenticada. Acesse novamente pelo Portal E3I.');
...
$('#analysisForm').addEventListener('submit', async event => {
  ...
  try {
    const response = await platformRequest('/v1/modules/suprimentos/pedidos-compra', {
      method: 'POST',
